...
- pinata(workbench)는 IPFS(sql)를 위한 기본 설정이 되어 있는 아이다.(중간 단계)
- IPFS 해보기 pinata(IPS)에 올려서 IPFS(탈중앙화인 P2P형식으로 저장하는 저장소 | 데이터보다 파일을 저장한다)에 저장했다. | NFT 저장하는데에 사용한다.
- S3는 IPFS와 같은 파일 저장소로 중앙화 방식이다. S3에다가는 빠르게 띄우기 위해서 사용한다. S3의 유효성을 검증하려면 IPFS를 가져와서 비교해본다.
...
// - Strings.sol 라이브러리는 위 기능을 편하게 제공한다.
...
contract nftToken is ERC721Enumerable, Ownable {
  ...
  // 4 * 4 이중배열(행열) | Rank 4, Type 4 설정하겠다(1 ~ 4까지만 사용하겠다.) >> 개수를 설정하기 나름(4는 정해진 내용은 아니다.)
  ...
  function mintToken() public payable {
    ...
    // 생성 시 이더를 받고 가격을 확인한다. | 돈받고 NFT 만들어줌
  ...
  function getRandomTokenData(
    ...
    // 때문에 유일한 값인 tokenId를 가져와서 암호화한 후 나머지 연산으로 0~99까지 랜덤한 수를 만든다.
  ...
  // memory vs calldata
  // memory는 수정 가능 calldata는 수정 불가능(복사도 불가능)
  // 저장 공간은 둘 다 임시 저장소(휘발성 데이터 Ram)
}
```
...
contract SaleToken {
  ...
  function SalesToken(uint _tokenId, uint _price) public {
    ...
    //  - 메타마스크를 연결했을 때 / 로그인했을 때 => 메타마스크의 계정에 대해 권한을 위임 받는다.(서명) | 메타마스크랑 연결했을 때 서명 받아서 메타마스크쪽에서 위임을 해준다.(메타마스크를 웹에 띄웠을때 권한을 위임한 것이다. | 서명 확인 누르는 것은 그 사람이 한 것인지 확인하기 위한 절차일 뿐)
    // OpenSea에 토큰에 대한 권한을 위임했는지 확인한다. (이미 상대방쪽에 구현되어있다.)

    tokenPrices[_tokenId] = _price;
    // 가격 등록(매핑 | 키와 값 매칭 시키는 것을 매핑)
  ...
  function cancelSaleToken(uint _tokenId) public {
    ...
    tokenPrices[_tokenId] = 0;
    popSaleToken(_tokenId);
    // 판매 목록에서 제외(판매했으니까 제외)
  }

  // 판매 등록부터 뜨는 것이 우선
  function popSaleToken(uint _tokenId) private returns (bool) {
    // 전달받은 토큰(tokenId)을 SaleTokenList에서 삭제 메서드
    // 만약 SaleTokenList = [1, 2, 3, 4, 5, 6] / 삭제할 토큰은 3
  ...
  function getOwnerTokens(
    ...
    // NFT 소유자 기준으로 등록한(갖고있는) NFT 목록 가져오기
    uint balance = Token.balanceOf(_tokenOwner);
    // 등록한(갖고있는) NFT 개수 가져오기
    require(balance > 0);
    // NFT 개수 확인, 없으면 멈춤

    TokenInfo[] memory list = new TokenInfo[](balance);
    // balance: owner가 갖고 있는 token의 잔액
  ...
  function getLatestToken(
    ...
    // 민팅 직후에 사용할 수 있다.
    uint balance = Token.balanceOf(_tokenOwner);
    uint tokenId = Token.tokenOfOwnerByIndex(_tokenOwner, balance - 1); // [1, 2, 3] => 마지막 NFT의 인덱스는? 2
    uint Rank = Token.getTokenRank(tokenId);
    uint Type = Token.getTokenType(tokenId);
    uint price = tokenPrices[tokenId];
...
- 민팅 => NftToken의 mintToken을 호출한다(ETH도 보내야한다.)
...
- 웹페이지에서 판매 NFT 목록 출력 => SaleToken 컨트렉트의 getSaleTokenList 메서드를 호출한다.
  ...
  - 출력해서 보여줘야지 사용자들이 판매 또는 구매 등등이 가능
