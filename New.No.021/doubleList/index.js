...
class DoubleLinkedList {
  ...
  insert(data) {
    ...
    if (!this.head) {
      ...
      console.log("헤드가 있다.");
      console.log("현제 테일의 다음 노드는 ", this.tail.next);
      this.tail.next = new Node(data);
      console.log("현제 테일의 다음 노드는 ", this.tail.next);
      // 끝에 다음 노드를 추가한다.
      this.tail.next.prev = this.tail;
      console.log("현제 테일의 다음 노드의 이전은 ", this.tail.next.prev);
  ...
  remove(data) {
    let curr = this.head;
    if (!curr) return;
    // if(curr?.data === data) 와 같은 구문이다.
    if (curr.data === data) {
      // ?.은 curr가 객체인지 확인하고 data 프로퍼티를 가져온다. (데이터를 가져올 수 있는지를 확인한다.)
