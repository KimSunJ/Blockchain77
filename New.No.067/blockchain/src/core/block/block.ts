...
class BlockHeader implements IBlockHeader {
  ...
  // class의 프로퍼티의 타입을 선언해주는 것이 아닌 정상적으로 type이 정의되었는가 확인한다.
  version: string;
  merkleRoot: string;
  timestamp: number;
  height: number;
  ...
  constructor(_data: Array<string>, _previousBlock?: IBlock) {
    ...
    if (merkleRoot.isError === true) {
      ...
      // value가 msg로 들어올 수도 있기 때문에 구분해준다.
  ...
  getDifficulty({
    ...
    // getDifficulty에 type은 return이 없어서 void
    if (this.height < DAI) {
      this.difficulty = 0;
...
class Block extends BlockHeader implements IBlock {
  ...
  constructor(
    _data: Array<string>,
    _previousBlock?: IBlock, // 이전 블록이 없을수도 있기 때문에
