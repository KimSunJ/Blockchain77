...
const {
  lib: { merkle, SHA256, hexToBinary }, // 명칭을 가지고 오려고 객체형식으로 가져옴
  constant: {
...
class Block extends IBlock {
  ...
  getDifficulty({
    ...
    // 높이가 난이도 조절 단위 개수 미만일 경우 최초 블록에서 현재까지 (난이도가 0)
    if (height < DIFFICULTY_ADJESTMENT_INTERVAL) return 0;

    // 높이가 난이도 조절 단위 개수의 2배 미만일 경우 최초 블록이 포함된 단위 개수 다음 개수
    ...
    // 현재 시간과 난이도 조절 단위 개수 이전의 시간의 차를 확인 (10개 이전의 시간)
    ...
    // 설정된 블록 10개 생성 시간보다 0.5배 미만이면 난이도 증가, 300초(5분)보다 작은 경우

    // 난이도 감소(난이도가 -가 될 수 없다.)
  ...
  updateBlock(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp) {
    ...
    while (!hashBinary.startsWith("0".repeat(this.difficulty))) {
      ...
      // startsWith : 특정문자열로 시작하는지 확인 (ex))
      // 난이도를 구해야 하니까 블록의 난이도를 전달해주고
      // 난이도가 2면 0이 두개("00")인지 확인
      // 블록의 난이도 갯수만큼 "0"이 붙을때까지 반복
      this.nonce += 1;
      // nonce를 증가시키고,
      this.timestamp = Date.now();
      // 현재 시간 가져오고 블록 생성 시점이 달라졌으므로 현재 시간을 다시 설정
      this.difficulty = this.getDifficulty(
        // 난이도를 다시 구한다.
        // 이전 블록 시간과
  ...
  isValidNewBlock(_newBlock, _previousBlock) {
    ...
    if (this.createHash(_newBlock) !== _newBlock.hash) {
      // hash를 다시 생성하여 생성된 블록의 hash와 맞는지 확인(생성된 블록의 hash값이 정확한지 확인)
