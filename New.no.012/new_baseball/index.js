...
// (배열과는 다르다, 쉽게 설명하려고 배열이라 한다. / 연속적인 데이터라는 것에선 같다)
// 사용할 때는 new Set()를 변수에 정의하여 변수를 사용한다. (let *** = [])
// clear() -> 매서드, add() -> 매서드, size -> 프롬퍼티
function reset() {
  comSel = [];
  comSet.clear();
  count = 0;
  ...
  //   console.log(comSet);
  comSel = [...comSet];
  // ...은 spread(스프레드_전개구문)이라고 한다.
  // 배열, 객체 등 연속되는 데이터들을 하나하나 분해한다.
  // [...comSet]은 comSet의 아이템 하나하나를 분해해서 배열 내에 넣어준다.
  // if(comSet.size == 3) 만약에 comSet.size가 3이면 [comSet[0], comSet[1], comSet[2]]와 같은 말이다.

  //   console.log(...comSet);
  console.log(comSel);
  alert("컴퓨터는 준비를 끝냈다 맞춰보거라 엣헴");
  // 컴퓨터 선택 끝
}
...
function selectNum() {
  ...
  //  !comSel -> 입력값이 없으면 리턴한다는 뜻
  let playerSel = "";
  let strike = 0;
  let ball = 0;

  //  html 찾아서 입력 받을 수 있도록 미리 정의해두자
  ...
  //   playerSel = ""; << 함수 내에서 (지역 변수로) 선언했기 때문에 굳이 안해도 됨
  //  플레이어의 선택 삭제
  if (strike === 3) {
    alert(`${count}번 만에 맞추셨네요.`);
    comSel = [];
  } else {
