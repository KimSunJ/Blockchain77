...
// Math.ranhom 메서드는 0 =< number && number <1
function comSelectFunc() {
  for (
    let tempNum = parseInt(Math.random() * 10);
    comSel.length < 3;
    tempNum = parseInt(Math.random() * 10) // 초기화 //  변화시키지 않아서 같은 숫자로 무한대 반복됨
  ) {
    if (!comSel.includes(tempNum)) comSel.push(tempNum);
    // includes 메서드는 매개변수가 배열에 포함되어 있는지 Boolean 값을 반환(리턴) 해준다 / 중복방지
  }
}
// for(변수 초기화; 조건; 반복 후 변경점){내용} / 조건이 성립되면 계속 반복되는 구문)
// 1. for는 "변수 초기화"를 실행하고, 조건을 확인 후 내용을 진행한다.
// 2. 내용 실행이 끝나면 "반복 후 변경점" 코드를 실행한다.
// 3. "반복 후 변경점" 코드 실행 후 조건을 확인하여 true면 내용을 실행한다.
// 이후 2로 돌아가서 다시 진행한다(조건이 false가 될때 까지 반복)
function comSelectFunc() {
  // for의 변수 초기화와 반복 후 변경점을 삭제했다.
  for (; comSel.length < 3; ) {
    const tempNum = parseInt(Math.random() * 10);
    // for 반복 내용에 tempNum를 초기화하여 comSel 배열을 확인 후 배열에 추가하도록 수정했다.
    // 위에서 (comSelectFunc)에서 for 변수 초기화는 let인데 여기서는 왜 const인가?
...
// comSelectFunc1();
comSelectFunc2();
// 함수를 호출함으로써 컴퓨터의 숫자를 3개 정한다. 3개의 함수 중 뭘 선택해도 상관 없음
...
while (true) {
  ...
  while (playerSel.length < 3) {
    //length는 string에 대한 길이를 인식한다.
    const tempInput = prompt("3자리의 숫자\n0으로 시작 가능 / 중복 숫자 불가");
    //   입력창에 대해서 취소를 누르면 tempInput은 정의되지 않아 undefined를 갖게 된다.
    // 그래서 length를 갖지 못해 if에서 문제가 생기게 된다.
    if (!tempInput) {
      // tempInput이 undefined, ''일 때 false를 갖게 되며
      // !를 사용해 부정함으로써 빈값을 입력하거나 취소를 누를 시 게임을 종료한다.
      alert("게임을 종료합니다.");
      break;
    }
    //  아무값이 없으면 false 다.
    if (tempInput.length != 3 || isNaN(parseInt(tempInput))) {
      ...
      // isNaN(parseInt(tempInput) -> 숫자가 아니면
      continue;
      // 아래의 코드를 실행하지 않고 반복을 다시 시작한다.
      // 다른 반복문들은 밑에 조건들을 확인 후 다시 실행하지만 밑에 코드를 실행하지 않고 반복문 안에서 반복되어야하기 때문에 사용
    }
    // else 사용한 것과 같음 (continue;)

    tempInput.split("").forEach((item) => {
      // forEach는 배열의 아이템을 하나하나 가져와서 매개변수함수에 전달하여 함수를 실행한다.
      // (item) => {내용} << 화살표 함수이다.
      // item은 tempInput.split('') 배열의 하나하나의 아이템이다.
      // function(item){내용}으로 대체 가능 (80~81줄)
      if (!playerSel.includes(parseInt(item))) playerSel.push(parseInt(item));
      // 컴퓨터가 선택한 랜덤 수는 정수이기 때문에 (위의 코드 참고 / tempNum = parseInt(Math.random() * 10);)
      // 플레이어가 입력한 숫자 또한 정수로 정의한다.
      // 컴퓨터가 선택한 랜덤 수와 마찬가지로 중복되면 안되기 때문에 playerSel 배열에 숫자가 없는지 확인 후 추가한다.
    });
    //  중복체크
    //  playerSel.push([parseInt(item)]);}); -> 배열에다가 item을 추가
    ...
    //   //   tempArr이라는 변수로 배열을 정의한다. << 왜? 그때그때마다 문자열(tempInput)을 split 메서드를 사용하여 자르면 시간이 그만큼 늘어난다.
    ...
    //  3자리 숫자가 아닐 경우 누적되는 숫자를 방지하기 위하여 초기화 시킨다.
  }
...
// while (true) {
// 적어도 값을 넣어줘야하기 때문에 true를 넣어야 된다.
// strike의 넣고싶으면 while문 밖에서 선언해야 된다.
// break;
// 왜 break;로 끊는지, true일때만 break로 멈출 수 있는지
// break와 true는 아무 관계 없음
// }

// let tempCount = 0;
// while (tempCount < 100) {
//   console.assert(++tempCount);
//   if (tempCount == 10) break;
// }
// if문을 통해서 10이면 멈춰라 해서 멈춤
