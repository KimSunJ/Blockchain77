...
contract EthSwap {
  ...
  function getToken() public view returns (address) {
   ...
   // 토큰의 CA를 반환한다.(상대방 입장에서도 토큰을 바로 받아서 사용 가능하다.)
  }
  ...
  function getThisAddress() public view returns (address) {
    ...
    return address(this);
    // 여기서 this는 해당 컨트렉트를 의미(this == EthSwap)
  ...
  function getTokenOwner() public view returns (address) {
    return token._owner;
    // 토큰 배포자를 반환한다. 즉, 토큰에 대한 스마트 컨트랙트 등록자(최초 발행자 리턴)
  }
...
- 비트코인은 스마트 컨트렉트가 존재하지 않기때문에 데이터 자체가 없다.
