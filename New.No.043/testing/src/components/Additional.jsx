...
export default function Additional() {
  ...
  useEffect(() => {
    ...
    // Element에 접근 시 current 프로퍼티로 가져온다.
  }, []);

  const [testValue, setValue] = useState(0);
  // 기존에 useEffect를 사용할 경우 어떤 상태(a)의 값이 변화 시 a를 기준으로 다른 상태(b)의 값을 계산하여 정의하는 경우가 있다.
  // ex) axios를 통한 데이터를 받기
  ...
  // 즉, testFunc는 함수이므로 호출할 때는 testFunc()로 사용해야 한다.
  ...
  // 즉, 필요없는 코드의 실행을 막을 수 있기 때문에 최적화 시 사용한다.
  // useMemo 또한 마찬가지로 최적화 시에 사용한다.

  return (
    <div
      id="now-test"
      className="now-class-test"
