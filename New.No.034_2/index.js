...
app.get("/search", async (req, res) => {
  // text/html > 문자열로 되어 있는 html 형식
  const data = (await axios.get("http://localhost:8080")).data;
  // axios.get("http://localhost:8080") 데이터를 가져올때까지 await(기다렸다가) data를 가져온다는 것
  res.send(data);
});
// await 서버에 요청을 보내고 서버쪽에서 보내는 값.

app.get("/search/query", async (req, res) => {
  // /search/query ? zxasd=asdasd << req.query = {zxasd: asdasd}
