...
const createMerkleRoot = () => {
  ...
  // SHA256(data[0]) : 첫번째 문자열의 데이터를 암호화한다. 해당 암호화된 데이터는 객체형식이다.
  // .toString() : 문자열로 변환한다.
  // .toUpperCase() : SHA256를 쓸 경우 영어가 소문자로 나타나기 때문에 대분자로 수정해준다.
};
...
// 데이터의 모든 정보(아이템)을 한번씩 hash(SHA256) 방식으로 암호화한다. / 각각 암호화
...
// createMerkleRoot() 함수를 createMerkleRoot에 담아 보낸다.
