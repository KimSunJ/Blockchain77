...
const asyncIncrement = () => {
  ...
  return async (dispatch, getState) => {
    ...
    try {
      // await를 사용한 함수의 호출이 에러가 발생할 수도 있기 때문에 (reject) try, catch 구문을 사용한다.
      ...
      //  7. result를 받아서 dispatch 호출하여 action으로 전달한다.
...
const asyncDecrement = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      // return await promiseTime(TYPE.DECREMENT, 1000); << state에 적용되지 않음
      dispatch({ type: result });
    } catch (error) {
      dispatch({ type: "error", payload: error });
    }
