const promiseTime = (type, time) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 만약에 서버와의 통신 (axios 등)을 사용할 시 setTimeout이 아니라 axios의 then 등을 이용해서 resolve 메서드를 호출한다.
    } catch (error) {
...
// 동기 처리는 실제 코드를 읽듯이 순서대로 처리한다.
//  - 일반 코드와 같음(위에서부터 순서대로)
...
//   }, 1000);
// }, 1000);
// 1초 마다 한번씩 실행한다. >> 콜백지옥이라고 한다.

// 아래의 방식으로 작성하는 경우가 많기 때문에 콜백지옥이라고 부른.
...
// callbackPromise("promise1", 1000)
//   .then((result) => {
// then은 promise 성공 시 실행하는 콜백함수를 전달한다.
// 콜백함수의 매개변수는 76번 줄의 resolve(text를 result로)에 전달된 매개변수를 받는다.
// console.log(result);
// return callbackPromise("promise2", 1000);
// 다음 프라미스(작업)은 return으로 반환하며 아래에 then으로 위의 프라미스(현재는 89번 줄)의 결과를 받을 수 있다.
//   })
//   .then((result) => {
// console.log(result);
// return callbackPromise("promise3", 1000);
//   })
//   .then((result) => {
// console.log(result);
// return callbackPromise("promise4", 1000);
//   })
//   .then((result) => {
// console.log(result);
// return callbackPromise("promise5", 1000);
//   })
//   .then((result) => {
// console.log(result);
//   })
